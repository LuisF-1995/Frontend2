...
fetch('https://randomuser.me/api/')
    ...
    .then(data => {
        ...
        console.log(data)
        //intento tomar los datos especificos para luego renderizarlos
        let email = data.results[0].email;
        let nombre = data.results[0].name.first + " " + data.results[0].name.last;
        let foto = data.results[0].picture.thumbnail;

        function renderizarDatosUsuario(correo, name, img) {
            /* -------------------------------- CONSIGNA 1 -------------------------------- */
            ...
            let tarjetaUsuario = document.querySelector(".tarjeta");
        
            const plantillaUsuario = ` <p>Nombre= ${name}</p>
                                        <p>e-mail= ${correo}</p> 
                                       <img src="${img}" alt="Foto" align="center">` ;
            
            tarjetaUsuario.innerHTML = plantillaUsuario;
        }
        renderizarDatosUsuario(email, nombre, foto);
